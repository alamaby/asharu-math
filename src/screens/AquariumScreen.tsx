...
import type { MathProblem, SessionSummary } from '../types'
import CheerfulAquarium from '../components/aquarium/CheerfulAquarium'
import { generateAquariumSession } from '../lib/aquariumQuestionGenerator'
...
export default function AquariumScreen({ levelId }: AquariumScreenProps) {
  ...
  const screenTitle = level ? level.name[lang] : t('aquarium.title')
  ...
  useEffect(() => {
    ...
    window.addEventListener('aquarium:toggle-sound', handler as EventListener)
    return () => window.removeEventListener('aquarium:toggle-sound', handler as EventListener)
  ...
  if (problems.length === 0) {
    return (
      <div className="py-10 text-center">
        ...
        <p className="mt-4 text-sm font-bold text-slate-500">Menyiapkan akuarium...</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-center">
        <button
          type="button"
          onClick={() => {
            setProblems(generateAquariumSession(levelId, 5))
            setIndex(0)
            setResults([])
