  ...
  return (
    ...
    >
      <div className="flex gap-1 text-[0.65rem] font-black uppercase tracking-wide">
        <span
          className={`px-2 py-1 text-sky-700 ${tensHighlight ? 'rounded-full bg-sky-100 ring-2 ring-sky-300' : ''}`}
        >
          Puluhan · P
        </span>
        <span
      ...
      <div className="flex gap-1">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl border-2 border-sky-100 bg-sky-50 text-xl font-black text-sky-700">
      ...
      <div className="flex gap-1">
        ...
        <div className="flex h-11 w-8 items-center justify-center text-2xl font-black text-slate-700" aria-hidden="true">
          {op}
        </div>
      </div>
      ...
      <div className="flex gap-1">
        {onSelectTens ? (
          ...
          <div className={answerClass(tensActive, wrongTens, tensHighlight)}>{tensAnswer ?? '·'}</div>
        )}
        {onSelectOnes ? (
          <button
            type="button"
          ...
          <div className={answerClass(onesActive, wrongOnes, onesHighlight)}>{onesAnswer ?? '·'}</div>
        )}
      </div>
    </div>
  )
